...
sudo bash -c "echo 'refclock SHM 0 refid NMEA offset 0.000 precision 1e-3 poll 0 filter 3'>>/etc/chrony/chrony.conf"
...
sudo bash -c "echo 'refclock PPS /dev/pps0 refid PPS lock NMEA offset 0.0 poll 3 trust'>>/etc/chrony/chrony.conf"
