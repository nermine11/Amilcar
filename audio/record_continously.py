...
#double buffering to avoid audio loss while saving at shutdown
...
def save_audio_and_markers(buffer_data, markers, filename: str):
    ...
    for timestamp, sample_offset, gps_position in markers:
        ...
        label = f"sample {sample_offset} at {time.strftime('%Y-%m-%d %H-%M-%S', time.gmtime(timestamp))}.{ms:03d}"
        if gps_position:
...
signal.signal(signal.SIGTERM, handle_exit)  # sudo halt, systemd, etc.
...
with client:
    ...
    #intial filename
    output_filename = get_current_filename()

    try:
        #checks if a shutdown flag has been raised(CTRL c or system shutdown)
        while not event.is_set():
            time.sleep(1)
    except KeyboardInterrupt:
        event.set() # shutdown will be called

#if shutdown has been triggered
with buffer_lock:
